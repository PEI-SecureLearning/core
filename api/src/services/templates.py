    ...
    def validate_template_requirements(self) -> "TemplateCreate":
        if self.path == "/templates/emails/":
            ...
            if "${{redirect}}" not in self.html or "${{pixel}}" not in self.html:
                raise ValueError(
                    "Email templates must include both ${{redirect}} and ${{pixel}}."
                )
        elif self.path == "/templates/pages/":
            if "${{redirect}}" not in self.html:
                raise ValueError(
                    "Landing page templates must include the ${{redirect}} variable."
                )
            self.subject = None
        return self
...
async def list_templates(
    org_id: str, include_platform: bool = False
) -> List[TemplateOut]:
...
async def get_template(
    template_id: str, org_id: str, can_view_platform: bool = False
) -> TemplateOut:
    collection = get_templates_collection()
    try:
...
async def update_template(
    template_id: str, template: TemplateUpdate, org_id: str
) -> TemplateOut:
    collection = get_templates_collection()
    try:
        oid = to_object_id(template_id)
    ...
    doc = await collection.find_one_and_update(
        filter_q, {"$set": payload}, return_document=ReturnDocument.AFTER
    )
