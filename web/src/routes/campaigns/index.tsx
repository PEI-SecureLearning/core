...
const statusConfig = {
  active: { label: "Active", color: "text-emerald-600", bg: "bg-emerald-500/10 backdrop-blur-sm border border-emerald-500/20", Icon: Play },
  completed: { label: "Completed", color: "text-blue-600", bg: "bg-blue-500/10 backdrop-blur-sm border border-blue-500/20", Icon: CheckCircle },
  scheduled: { label: "Scheduled", color: "text-amber-600", bg: "bg-amber-500/10 backdrop-blur-sm border border-amber-500/20", Icon: Clock },
  paused: { label: "Paused", color: "text-slate-600", bg: "bg-slate-500/10 backdrop-blur-sm border border-slate-500/20", Icon: Pause },
  failed: { label: "Failed", color: "text-rose-600", bg: "bg-rose-500/10 backdrop-blur-sm border border-rose-500/20", Icon: XCircle },
};
...
function GlassDropdown({ value, onChange, options }: GlassDropdownProps) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <span className="text-[14px] font-medium text-slate-600">{selectedLabel}</span>
...
function CampaignsPage() {
  ...
  const totalEmailsSent = mockCampaigns.reduce((acc, c) => acc + c.stats.sent, 0);
    ...
    <div className="p-8 space-y-8 font-[Inter,system-ui,sans-serif] min-h-screen bg-gradient-to-br from-slate-50 via-white to-purple-50/30 min-w-[320px] sm:min-w-[640px] md:min-w-[768px] lg:min-w-[1024px] xl:min-w-[1280px] animate-[fadeIn_0.5s_ease-out]">
      ...
      <div className="h-[5%] flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1.5 text-[15px] font-normal">Manage your phishing simulation campaigns</p>
        </div>
        <Link
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-5 h-auto md:h-[10%] min-w-0">
        <div className="h-full bg-white/60 backdrop-blur-xl p-5 rounded-2xl border border-white/40 shadow-lg shadow-slate-200/50 hover:shadow-xl hover:shadow-slate-200/60 transition-all duration-300 hover:-translate-y-0.5">
          <p className="text-[13px] font-medium text-slate-500 tracking-wide uppercase">Total Campaigns</p>
          <p className="text-3xl font-semibold text-slate-900 mt-1.5 tracking-tight">{totalCampaigns}</p>
        </div>
        <div className="bg-white/60 backdrop-blur-xl p-5 rounded-2xl border border-white/40 shadow-lg shadow-slate-200/50 hover:shadow-xl hover:shadow-emerald-200/40 transition-all duration-300 hover:-translate-y-0.5">
          <p className="text-[13px] font-medium text-slate-500 tracking-wide uppercase">Active</p>
          <p className="text-3xl font-semibold text-emerald-600 mt-1.5 tracking-tight">{activeCampaigns}</p>
        </div>
        <div className="bg-white/60 backdrop-blur-xl p-5 rounded-2xl border border-white/40 shadow-lg shadow-slate-200/50 hover:shadow-xl hover:shadow-purple-200/40 transition-all duration-300 hover:-translate-y-0.5">
          <p className="text-[13px] font-medium text-slate-500 tracking-wide uppercase">Total Emails Sent</p>
          <p className="text-3xl font-semibold text-purple-600 mt-1.5 tracking-tight">{totalEmailsSent.toLocaleString()}</p>
        </div>
        <div className="bg-white/60 backdrop-blur-xl p-5 rounded-2xl border border-white/40 shadow-lg shadow-slate-200/50 hover:shadow-xl hover:shadow-amber-200/40 transition-all duration-300 hover:-translate-y-0.5">
          <p className="text-[13px] font-medium text-slate-500 tracking-wide uppercase">Avg Click Rate</p>
          <p className="text-3xl font-semibold text-amber-600 mt-1.5 tracking-tight">13%</p>
        </div>
      </div>
      ...
      <div className="h-[55%] mt-13 bg-white/60 backdrop-blur-xl rounded-2xl border border-white/40 shadow-lg shadow-slate-200/50 flex flex-col overflow-hidden">
        ...
        <div className="bg-slate-50/80 backdrop-blur-sm border-b border-slate-200/60 flex-shrink-0">
          <table className="w-full">
            <thead>
              <tr>
                <th className="text-left px-6 py-4 text-[11px] font-semibold text-slate-500 uppercase tracking-wider w-[22%]">Campaign</th>
                <th className="text-left px-6 py-4 text-[11px] font-semibold text-slate-500 uppercase tracking-wider w-[12%]">Status</th>
                <th className="text-left px-6 py-4 text-[11px] font-semibold text-slate-500 uppercase tracking-wider w-[20%]">Duration</th>
                <th className="text-left px-6 py-4 text-[11px] font-semibold text-slate-500 uppercase tracking-wider w-[12%]">Sent</th>
                <th className="text-left px-6 py-4 text-[11px] font-semibold text-slate-500 uppercase tracking-wider w-[12%]">Opened</th>
                <th className="text-left px-6 py-4 text-[11px] font-semibold text-slate-500 uppercase tracking-wider w-[12%]">Clicked</th>
          ...
          <table className="w-full">
            <tbody className="divide-y divide-slate-100/60">
              {filteredCampaigns.map((campaign) => {
                ...
                return (
                  <tr key={campaign.id} className="hover:bg-slate-50/60 transition-colors duration-150">
                    <td className="px-6 py-4 w-[22%]">
                      <p className="font-medium text-[14px] text-slate-900 tracking-tight">{campaign.name}</p>
                      <p className="text-[13px] text-slate-500 truncate max-w-xs mt-0.5">{campaign.description}</p>
                    </td>
                    ...
                    <td className="px-6 py-4 w-[20%]">
                      <div className="flex items-center gap-2 text-[13px] text-slate-600">
                        ...
                        <span className="font-normal">{new Date(campaign.begin_date).toLocaleDateString()} - {new Date(campaign.end_date).toLocaleDateString()}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 w-[12%]">
                      <div className="flex items-center gap-2">
                        <Mail size={14} className="text-slate-400" />
                        <span className="font-semibold text-[14px] text-slate-900">{campaign.stats.sent.toLocaleString()}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 w-[12%]">
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-[14px] text-slate-900">{campaign.stats.opened.toLocaleString()}</span>
                        <span className="text-[12px] text-slate-400 font-normal">({openRate}%)</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 w-[12%]">
                      <div className="flex items-center gap-2">
                        <Globe size={14} className="text-amber-500" />
                        <span className="font-semibold text-[14px] text-slate-900">{campaign.stats.clicked}</span>
                        <span className="text-[12px] text-slate-400 font-normal">({clickRate}%)</span>
                      </div>
                    </td>
        ...
        {filteredCampaigns.length === 0 && (
          <div className="text-center py-16">
            <p className="text-slate-400 text-[15px] font-normal">No campaigns found</p>
          </div>
        )}
      </div>
    </div>
