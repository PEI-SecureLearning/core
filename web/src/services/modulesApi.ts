 ...
 * Used for partial updates if needed.
 */
...
export async function fetchModules(options?: {
  ...
  const { search, sort, page = 1, limit = 20, token } = options ?? {};
 ...
 * The API rejects attempts to delete published modules if they are in use by courses.
 */
