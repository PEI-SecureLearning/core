...
export const getRoleBadgeVariantProps = (role?: string | null): { variant: "outline" | "secondary"; className?: string } => {
    if (!role) return { variant: "outline" };

    const normalizedRole = role.toLowerCase().replaceAll("-", "_").replaceAll(" ", "_");

    if (normalizedRole === "org_manager") {
        return { variant: "secondary", className: "border-violet-500/20 bg-violet-500 text-white hover:bg-violet-500/90" };
    }
