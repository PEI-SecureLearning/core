...
export function renderMarkdown(md: string): string {
    ...
    out = out.replaceAll(/```(\w*)\n([\s\S]*?)```/g, (_m, lang, code) =>
        `<pre class="bg-muted text-success rounded-lg px-4 py-3 my-3 overflow-x-auto text-xs font-mono whitespace-pre">${lang ? `<span class="text-muted-foreground text-[10px] uppercase block mb-1">${lang}</span>` : ''
        }${code.trim()}</pre>`)

    // Blockquotes (lines starting with >)
    out = out.replaceAll(/^&gt; (.+)$/gm,
        '<blockquote class="border-l-4 border-accent-secondary pl-3 italic text-muted-foreground my-2">$1</blockquote>')
    ...
    out = out.replaceAll(
        ...
        (_m, header, rows) => {
            const thCells = header.split('|').filter((c: string) => c.trim()).map((c: string) =>
                `<th class="px-3 py-2 text-left text-xs font-semibold text-foreground bg-surface-subtle border border-border">${c.trim()}</th>`
            ).join('')
            const bodyRows = rows.trim().split('\n').map((row: string) => {
                const tds = row.split('|').filter((c: string) => c.trim()).map((c: string) =>
                    `<td class="px-3 py-2 text-xs text-foreground/90 border border-border">${c.trim()}</td>`
                ).join('')
                return `<tr>${tds}</tr>`
            }).join('')
    ...
    // Headings
    out = out
        .replaceAll(/^### (.+)$/gm, '<h3 class="text-base font-bold mt-4 mb-1 text-foreground/90">$1</h3>')
        .replaceAll(/^## (.+)$/gm, '<h2 class="text-lg font-bold mt-5 mb-2 text-foreground/90">$1</h2>')
        .replaceAll(/^# (.+)$/gm, '<h1 class="text-xl font-bold mt-6 mb-2 text-foreground">$1</h1>')

    // Inline: bold, italic, strikethrough, inline-code
    out = out
        .replaceAll(/\*\*(.+?)\*\*/g, '<strong class="font-semibold text-foreground">$1</strong>')
        .replaceAll(/~~(.+?)~~/g, '<del class="text-muted-foreground line-through">$1</del>')
        .replaceAll(/_(.+?)_/g, '<em class="italic">$1</em>')
        .replaceAll(/`([^`]+)`/g, '<code class="bg-muted text-primary rounded px-1 py-0.5 text-[12px] font-mono">$1</code>')

    // Lists
    out = out
        .replaceAll(/^- (.+)$/gm, '<li class="ml-5 list-disc text-foreground/90 leading-relaxed">$1</li>')
        .replaceAll(/^\d+\. (.+)$/gm, '<li class="ml-5 list-decimal text-foreground/90 leading-relaxed">$1</li>')

    // Plain paragraphs (lines not already wrapped in an HTML tag)
    out = out.replaceAll(/^(?!<[a-zA-Z/])(.+)$/gm,
        '<p class="text-foreground/90 leading-relaxed my-1">$1</p>')
