...
import {
    BookOpen, Check, CheckCircle, ChevronDown, ChevronRight, Clock, FileText,
    Image as ImageIcon, Layers, ListChecks, Lock,
    PartyPopper, SkipForward, X,
} from 'lucide-react'
import type { CourseModule } from './courseData'
import { COURSES } from './courseData'
import type { Block, Choice, Section } from '@/components/content-manager/modules/module-creation/types'
...
function QuestionBlock({ block, qIndex, answeredChoices, onMark }: {
    ...
    const choiceBtnClass = (c: Choice) => {
        if (answered !== c.id) {
            return 'bg-white border-slate-200 text-slate-700 hover:bg-purple-50/50 hover:border-purple-200'
        }
    ...
    return (
        <div className="border border-slate-200 rounded-xl overflow-hidden bg-white shadow-sm">
            <div className="flex items-center gap-2 px-4 py-2.5 bg-slate-50 border-b border-slate-100">
                <ListChecks className="w-3.5 h-3.5 flex-shrink-0 text-purple-500" />
            ...
            <div className="px-5 py-4">
                ...
                {q.type === 'short_answer' ? (
                    <div className="flex gap-2">
                        ...
                        <button type="button"
                            className="px-4 py-2 text-white text-sm font-semibold rounded-lg transition-colors bg-purple-600 hover:bg-purple-700">
                            Check
                        </button>
                    </div>
                ) : (
...
function PreviewBlock({ block, qIndex, answeredChoices, onMark }: {
    ...
    if (block.kind === 'question') {
        return <QuestionBlock block={block} qIndex={qIndex} answeredChoices={answeredChoices} onMark={onMark} />
    }
    return null
}
...
function isSectionComplete(
    ...
    const questions = section.blocks.filter(b => b.kind === 'question') as Extract<Block, { kind: 'question' }>[]
...
function Toast({ message, onDone }: { message: string; onDone: () => void }) {
    return (
        <motion.div
            ...
            className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-slate-900 text-white px-5 py-3 rounded-xl shadow-2xl text-sm font-medium flex items-center gap-2"
        >
...
export default function ModuleLearner({ module: mod, courseId }: Props) {
    ...
    const [collapsedSections, setCollapsedSections] = useState<Record<string, boolean>>(() => {
        ...
        sections.forEach((s, i) => { collapsed[s.id] = i !== 0 })
        return collapsed
    })
    const [answeredChoices, setAnsweredChoices] = useState<Record<string, string>>({})
    const [completedSections, setCompletedSections] = useState<Set<string>>(new Set())
    const [toastMsg, setToastMsg] = useState<string | null>(null)
    const [openedSections, setOpenedSections] = useState<Set<string>>(() => new Set(sections[0] ? [sections[0].id] : []))
    ...
    const allComplete = sections.every(s => s.isOptional || completedSections.has(s.id))
    ...
    return (
        <div className="fixed inset-0 z-50 flex flex-col h-full overflow-hidden bg-slate-50">

            {/* ── Top bar (mirrors ModulePreview) ── */}
            <div className="relative flex items-center justify-between px-6 py-4 bg-white border-b border-slate-200 shadow-sm flex-shrink-0">
                {/* Left: back chevron + icon + title + meta */}
                <div className="flex items-center gap-4 min-w-0">
                    <Link
                        to={'/courses/$courseId' as any}
                        params={{ courseId } as any}
                        className="flex items-center gap-1 text-slate-400 hover:text-purple-600 transition-colors text-sm font-medium flex-shrink-0"
                    >
                    ...
                    <div className="flex items-center gap-20 flex-shrink-0">
                        {mod.difficulty && (['Easy', 'Medium', 'Hard'] as const).includes(mod.difficulty as 'Easy' | 'Medium' | 'Hard') && (
                            <span className={`px-2.5 py-0.5 rounded text-xs font-bold border ${DIFFICULTY_COLORS[mod.difficulty as 'Easy' | 'Medium' | 'Hard']}`}>
                                {mod.difficulty}
                            </span>
                        )}
                        {mod.estimatedTime && (
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div className="w-64 flex-shrink-0 bg-white border-r border-slate-200 flex flex-col overflow-hidden">
                        ...
                        <div className="mt-2">
                            <div className="flex items-center justify-between text-[10px] text-slate-400 mb-1">
                                ...
                                <span className="font-bold text-purple-600">
                                    {sections.length > 0
                                        ? Math.round((completedSections.size / sections.filter(s => !s.isOptional).length) * 100)
                                        : 0}%
                                </span>
                            </div>
                    ...
                    <div className="flex-1 overflow-y-auto py-2">
                        ...
                        {sections.length === 0
                            ? <p className="px-4 py-3 text-xs text-slate-400 italic">No sections</p>
                            : sections.map((sec, i) => {
                                ...
                                else if (isLocked) badgeCls = 'bg-slate-200 text-slate-400'

                                return (
                                    <button key={sec.id} type="button"
                                        onClick={() => scrollTo(sec.id)}
                                        className={`w-full text-left px-4 py-2.5 flex items-start gap-2.5 transition-colors group border-l-2 border-transparent
                                        ${isLocked
                                                ? 'cursor-not-allowed opacity-50'
                                                : 'hover:bg-slate-50 hover:border-purple-300 cursor-pointer'
                                            }`}
                                    >
                                        <span className={`mt-0.5 w-5 h-5 rounded flex items-center justify-center text-[10px] font-bold flex-shrink-0 ${badgeCls}`}>
                                            {isComplete ? <Check className="w-3 h-3" /> : isLocked ? <Lock className="w-3 h-3" /> : i + 1}
                                        </span>
                                        <div className="min-w-0">
                                            <p className={`text-xs font-semibold truncate transition-colors ${isLocked ? 'text-slate-400' : 'text-slate-700 group-hover:text-purple-700'}`}>
                        ...
                        return (
                            <div className="max-w-7xl mx-auto px-8 py-8 flex flex-col gap-4">
                                {sections.map((sec, i) => {
                                    ...
                                    const noQuestionsButOpened = qCount === 0 && openedSections.has(sec.id)
                                    const canComplete = !isComplete && !isLocked && (sectionDone || noQuestionsButOpened)
                                    ...
                                    return (
                                        <motion.div
                                            ...
                                            className={`bg-white rounded-2xl border shadow-sm overflow-hidden scroll-mt-4 transition-all
                                            ${isLocked
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                ${isComplete ? 'bg-emerald-500' : isLocked ? 'bg-slate-300' : 'bg-purple-600'}`}>
                                                    {isComplete ? <Check className="w-4 h-4" /> : isLocked ? <Lock className="w-4 h-4" /> : i + 1}
                                                </span>
                                                <div className="flex-1 min-w-0">
                                                    <p className={`text-base font-bold transition-colors truncate
                                                    ${isLocked ? 'text-slate-400' : isComplete ? 'text-emerald-700' : 'text-slate-800 group-hover:text-purple-700'}`}>
                                                        {sec.title || `Section ${i + 1}`}
                                                    </p>
                                                    <div className="flex items-center gap-2 mt-0.5">
                                                        ...
                                                        {sec.requireCorrectAnswers && (
                                                            <span className="text-[10px] text-green-500 font-medium flex items-center gap-0.5">
                                                                <CheckCircle className="w-3 h-3" /> Correct answers required
                                                            </span>
                                                        )}
                                                        {qCount > 0 && (
                                                ...
                                                {!isLocked && (
                                                    <motion.div
                                                        ...
                                                        className="text-slate-400 flex-shrink-0 group-hover:text-purple-500"
                                                    >
                                                ...
                                                {!isCollapsed && !isLocked && (
                                                    ...
                                                    >
                                                        <div className="px-6 pb-6 pt-1 flex flex-col gap-5 border-t border-slate-100">
                                                            {sec.blocks.length === 0 ? (
                                                                <p className="text-sm text-slate-400 italic py-3">No content in this section yet.</p>
                                                            ) : (() => {
                                                                ...
                                                                {!isComplete && (
                                                                    <>
                                                                        <button
                                                                            ...
                                                                            ${canComplete
                                                                                    ? 'bg-purple-600 text-white hover:bg-purple-700 shadow-md hover:shadow-lg active:scale-[0.97]'
                                ...
                                <AnimatePresence>
                                    {allComplete && (
                                        <motion.div
                                            ...
                                            className="bg-gradient-to-br from-purple-600 to-purple-800 rounded-2xl p-8 text-center text-white shadow-xl"
                                        >
                                            ...
                                            <Link
                                                to={'/courses/$courseId' as any}
                                                params={{ courseId } as any}
                                                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white text-purple-700 font-semibold shadow-md hover:shadow-lg hover:bg-purple-50 transition-all"
                                            >
                                                ← Back to Course
                                            </Link>
                                        </motion.div>
