    ...
    return (
        <div className="border border-border rounded-xl overflow-hidden bg-background shadow-sm">
            <div className="flex items-center gap-2 px-4 py-2.5 bg-surface-subtle border-b border-border/40">
                <ListChecks className="w-3.5 h-3.5 flex-shrink-0 text-primary/90" />
                <span className="text-[10px] font-bold uppercase tracking-widest text-primary/90">{typeLabel}</span>
                <span className="ml-auto text-[10px] text-muted-foreground/70">Q{qIndex}</span>
            </div>
            <div className="px-5 py-4">
                <p className="text-sm font-semibold text-foreground leading-relaxed mb-4">
                    {q.text || <em className="text-muted-foreground/70 font-normal">No question text</em>}
                </p>
                {q.type === 'short_answer' ? (
                        ...
                        {choices.map(c => {
                            ...
                            return (
                                <button key={c.id} type="button"
                                    ...
                                    <span className="flex-1">{c.text || <em className="text-muted-foreground/70">Empty choice</em>}</span>
                                </button>
                            )
                        })}
                    </div>
...
export default function ModuleLearner({ module: mod, courseId }: Props) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex flex-col h-full overflow-hidden bg-surface-subtle">
            ...
            <div className="relative flex items-center justify-between px-6 py-4 bg-background border-b border-border shadow-sm flex-shrink-0">
                {/* Left: back chevron + icon + title + meta */}
                <div className="flex items-center gap-4 min-w-0">
                    <Link
                        to={'/courses/$courseId' as any}
                        params={{ courseId } as any}
                        className="flex items-center gap-1 text-muted-foreground/70 hover:text-primary transition-colors text-sm font-medium flex-shrink-0"
                    >
            ...
            <div className="flex flex-1 overflow-hidden">
                {/* ── Left sidebar ── */}
                <div className="w-64 flex-shrink-0 bg-background border-r border-border flex flex-col overflow-hidden">
                    {/* Module title + meta */}
                    ...
                    <div className="flex-1 overflow-y-auto py-2">
                        ...
                        {sections.length === 0
                            ? <p className="px-4 py-3 text-xs text-muted-foreground/70 italic">No sections</p>
                            : sections.map((sec, i) => {
                                ...
                                else if (isLocked) badgeCls = 'bg-muted/60 text-muted-foreground/70'
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="min-w-0">
                                            <p className={`text-xs font-semibold truncate transition-colors ${isLocked ? 'text-muted-foreground/70' : 'text-foreground/90 group-hover:text-primary'}`}>
                                            ...
                                            >
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="flex items-center gap-2 mt-0.5">
                                                        ...
                                                        {isLocked && (
                                                            <span className="text-[10px] text-muted-foreground/70 font-medium">🔒 Locked</span>
                                                        )}
                                                    </div>
                                                </div>
                                                {!isLocked && (
                                                    <motion.div
                                                        ...
                                                        className="text-muted-foreground/70 flex-shrink-0 group-hover:text-primary/90"
                                                    >
                                                ...
                                                {!isCollapsed && !isLocked && (
                                                    ...
                                                    >
                                                        <div className="px-6 pb-6 pt-1 flex flex-col gap-5 border-t border-border/40">
                                                            {sec.blocks.length === 0 ? (
                                                                <p className="text-sm text-muted-foreground/70 italic py-3">No content in this section yet.</p>
                                                            ) : (() => {
